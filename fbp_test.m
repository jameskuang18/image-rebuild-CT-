...
figure,imshow(R_ifft),title('逆傅里叶变换');
...
for i = 1:180
    ...
    for x = 1:N
        for y = 1:N
            %{
            %最近邻插值法
            t = round((x-N/2)*cos(rad)-(y-N/2)*sin(rad));%将每个元素舍X入到最接近的整数。
            if t<size(R,1)/2 && t>-size(R,1)/2
                fbp(x,y)=fbp(x,y)+proj_ifft(round(t+size(R,1)/2),i);
            end
            %}
...
fbp=(fbp*pi)/180;%原图每个像素的密度值

%% 显示结果
xlswrite('rebuild_info_test2.xlsx',fbp,'Sheet1');%将得到的重建后的图像数据写入
figure,imshow(fbp),title('反投影变换后的图像')%是否输出是头型图
